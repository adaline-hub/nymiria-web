#!/usr/bin/env bash
# scripts/install.sh — Linux installer for Nymiria
#
# Usage (one-liner, hosted at nymiria.com/install.sh):
#   curl -fsSL https://nymiria.com/install.sh | bash
#
# What it does:
#   1. Fetches the latest release tag from nymiria-updates
#   2. Downloads the AppImage and installs it to ~/Applications/Nymiria.AppImage
#   3. Creates a ~/.local/bin/nymiria symlink
#   4. In-app auto-update works on all distros with AppImage
...
case ":${PATH}:" in
  ...
  *)
    ...
    echo "Add this to your shell profile to run 'nymiria' from anywhere:"
